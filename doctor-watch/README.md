...
4. **비교** `diff.py` — 병원별 **의사 상태표**(`doctor_state`)로 판단합니다.
   - 새 이름은 **2회 연속 실행에서 보여야 '합류'**, 있던 이름은 **2회 연속 안 보여야 '제외'** 로 확정 (1주 지연, 대신 오탐 억제).
     '오늘의 의료진' 처럼 열 때마다 내용이 바뀌는 페이지에 잠깐 실린 이름은 변동이 되지 않습니다.
   - 그 사람이 실렸던 페이지를 이번 주에 못 읽었으면(타임아웃 등) 부재로 세지 않습니다.
   - 진료과 집합이 전혀 안 겹치면 '진료과 변경', 직위가 바뀌면 '직위 변경' (이것도 2회 연속 확인 후 확정).
   - 명단이 절반 이하로 급감하면 사이트 개편 가능성으로 "검토 필요", 1.5배 이상 급증하면 수집 범위 확대로 보고 기준선을 다시 잡습니다.
...
- **GitHub Pages**: 저장소가 Pages 를 켜 두었다면 매주 실행 후 `https://<계정>.github.io/<저장소>/doctor-watch/reports/index.html` 에
  브리핑이 게시되고, 같은 폴더의 `hospitals.html`(병원 목록) · `hospital-<id>.html`(병원별 명단·이력) · `search.html`(의사 검색) · `runs.html`(실행 이력)이 함께 올라갑니다.
  저장소가 공개(public)면 이 페이지도 공개되므로 유의하세요.
- **Actions artifact**: 각 실행의 `doctor-watch-report` 로 90일간 내려받을 수 있습니다.
- **로컬 대시보드**: `python -m doctor_watch serve` (수동 지정·고객명단 편집 등 쓰기 기능 포함).

## 알려진 한계

- **GitHub 러너는 미국 IP** 라 공공데이터포털(apis.data.go.kr)과 일부 병원 사이트(해외 IP 차단, 예: 대동병원 403)에 접속하지 못합니다.
  심평원 목록은 국내 PC 에서 `init-hospitals` 로 만들어 DB 를 커밋하거나 CSV 로 관리하면 되고,
  차단하는 병원이 많아지면 국내 PC/서버를 [self-hosted runner](https://docs.github.com/actions/hosting-your-own-runners) 로 등록해 같은 워크플로를 돌리면 됩니다.
